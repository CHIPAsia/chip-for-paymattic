 ...
 *
 */
if ( ! class_exists( 'CHIPPYMTC_Comment_Metabox' ) ) {
  class CHIPPYMTC_Comment_Metabox extends CHIPPYMTC_Abstract{
    ...
    public function add_comment_meta_box_content( $comment, $callback ) {
      ...
      $errors   = ( is_object ( $comment ) ) ? get_comment_meta( $comment->comment_ID, '_chippymtc_errors_'. $this->unique, true ) : array();
      $errors   = ( ! empty( $errors ) ) ? $errors : array();
      $theme    = ( $this->args['theme'] ) ? ' chippymtc-theme-'. $this->args['theme'] : '';
      $nav_type = ( $this->args['nav'] === 'inline' ) ? 'inline' : 'normal';

      if ( is_object( $comment ) && ! empty( $errors ) ) {
        delete_comment_meta( $comment->comment_ID, '_chippymtc_errors_'. $this->unique );
      }

      wp_nonce_field( 'chippymtc_comment_metabox_nonce', 'chippymtc_comment_metabox_nonce'. $this->unique );

      echo '<div class="chippymtc chippymtc-comment-metabox'. esc_attr( $theme ) .'">';
            ...
            foreach ( $this->sections as $section ) {
              ...
              if ( ! empty( $section['fields'] ) ) {
                ...
                foreach ( $section['fields'] as $field ) {
                  ...
                  CHIPPYMTC::field( $field, $this->get_meta_value( $comment->comment_ID, $field ), $this->unique, 'comment_metabox' );
    ...
    public function save_comment_meta_box( $comment_id ) {
      ...
      $data     = array();
      $errors   = array();
      $noncekey = 'chippymtc_comment_metabox_nonce'. $this->unique;
      $nonce    = ( ! empty( $_POST[ $noncekey ] ) ) ? sanitize_text_field( wp_unslash( $_POST[ $noncekey ] ) ) : '';

      if ( ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) || ! wp_verify_nonce( $nonce, 'chippymtc_comment_metabox_nonce' ) ) {
        return $comment_id;
      }
      ...
      $data = apply_filters( "chippymtc_{$this->unique}_save", $data, $comment_id, $this );

      do_action( "chippymtc_{$this->unique}_save_before", $data, $comment_id, $this );
      ...
      if ( empty( $data ) || ! empty( $request['_reset'] ) ) {
        ...
        if ( ! empty( $errors ) ) {
          update_comment_meta( $comment_id, '_chippymtc_errors_'. $this->unique, $errors );
        }

      }

      do_action( "chippymtc_{$this->unique}_saved", $data, $comment_id, $this );

      do_action( "chippymtc_{$this->unique}_save_after", $data, $comment_id, $this );
